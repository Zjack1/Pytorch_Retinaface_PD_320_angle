...
from utils.box_utils import decode, decode_landm_25
...
class Angle(nn.Module):
    def __init__(self, roi_size=20, spatial_scale=1):
        super(Angle, self).__init__()
        self.roi = RoIPool((roi_size, roi_size), spatial_scale)
        self.conv1 = conv_bn(32, 32, 1 )
        ...
        self.fc11 = nn.Linear(50, 64)
        self.fc21 = nn.Linear(64, 64)
        self.fc31 = nn.Linear(64, 64)
        self.fc41 = nn.Linear(64, 64)
        self.fc51 = nn.Linear(64, 64)
        self.fc61 = nn.Linear(64, 32)

        self.fc_rpy = nn.Linear(32, 3)

    def forward(self, x, rois, roi_indices, norm_landmarks):
        n, _, _, _ = x.shape
        if x.is_cuda:
        ...
        out7 = self.max_pool(out6)
        out8 = out7.view(out7.size(0), -1)
        out9 = self.fc1(out8)

        norm_landmarks = norm_landmarks.cuda()
        out11 = self.fc11(norm_landmarks)
        out21 = self.fc21(out11)
        out31 = self.fc31(out21)
        out41 = self.fc41(out31)
        out51 = self.fc51(out41)
        out61 = self.fc61(out51)

        concat_fc = out61 + out9
        rpy = self.fc_rpy(concat_fc)
...
def decode_face_box_out(out):
    ...
    for i in range(batch_size_n):
        ...
        box = box.cpu().numpy()
        landms = decode_landm_25(landmk[i].data.cuda(), prior_data, torch.tensor(cfg_mnet['variance']).cuda())
        ...
        landms = landms[order[0]]  # 取出分数最大的那个特征点

        landms1 = landms.view(-1, 2)
        landms1 = landms1.cpu().numpy()
        c_xy = (box[2:4] + box[:2]) / 2  # 中心点
        norm_landmark = (landms1 - c_xy) * 10
        tensor_norm_landmark = torch.from_numpy(norm_landmark)
        tensor_norm_landmark = tensor_norm_landmark.view(1, -1)
        norm_landmarks.append(tensor_norm_landmark)
    ...
    norm_landmarks = torch.cat(norm_landmarks, dim=0)

    return rois, roi_indices, norm_landmarks
...
class Retinaface_Angle_Net(nn.Module):
    ...
    def forward(self, inputs):
        ...
        rois, roi_indices, norm_landmarks = decode_face_box_out(face_out)
        angle_rpy = self.angle(backbone, rois, roi_indices, norm_landmarks)
