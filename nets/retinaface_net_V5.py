...
class RetinaFace(nn.Module):
    def __init__(self, cfg = None, phase = 'train'):
        ...
        if cfg['name'] == 'mobilenet0.25':
            backbone = MobileNetV1().to(device)
            if cfg['pretrain']:
                checkpoint = torch.load("./weights/retinaface_net_V4/data_130w/mobilenet0.25_epoch_13.pth", map_location=device)
                from collections import OrderedDict
            ...
            backbone = models.resnet50(pretrained=cfg['pretrain']).to(device)
...
class FPN(nn.Module):
    ...
    def forward(self, input):
        ...
        output1 = self.output1(input[1])  # 80x80x64 -> 80x80x64
        output2 = self.output2(input[2])  # 40x40x128 -> 40x40x64
        output3 = self.output3(input[3])  # 20x20x256 -> 20x20x64
        # 20x20x64 -> 40x40x64
