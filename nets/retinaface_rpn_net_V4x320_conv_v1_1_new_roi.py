...
class Angle(nn.Module):
    def __init__(self,roi_size=20, spatial_scale=1):
        ...
        self.conv8 = conv_bn(32, 32, 1)
        self.conv9 = conv_bn(32, 32, 2)
...
def decode_face_box_out(out):
    ...
    for i in range(batch_size_n):
        ...
        cx = box[0] + (w/2)
        cy = box[1] + (h/2)
        new_box = [cx-roi_h, cy-roi_h, cx+roi_h, cy+roi_h]
        new_box = np.array(new_box).astype("float32")#.reshape(4,-1)
        score = score[order[0]] # 取出分数最大的那个分数
        tensor_roi = torch.from_numpy(new_box).unsqueeze(0)
