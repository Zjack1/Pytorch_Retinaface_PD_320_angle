...
import datetime
import math
from nets.retinaface_rpn_net_V4x320_conv_v1_1_new_roi import Retinaface_Angle_Net
...
parser.add_argument('--resume_net', default="./weights/27w_conv_v1_1_new_roi/Angle_V4x320_conv_v1_1_27w_epoch_90_best.pth", help='resume net for retraining')
parser.add_argument('--resume_epoch', default=0, type=int, help='resume iter for retraining')
parser.add_argument('--weight_decay', default=1e-5, type=float, help='Weight decay for SGD')
parser.add_argument('--gamma', default=0.1, type=float, help='Gamma update for SGD')
parser.add_argument('--save_folder', default='./weights/27w_conv_v1_1_new_roi/', help='Location to save checkpoint models')
    ...
    for iteration in range(start_iter, max_iter):
        if iteration % epoch_size == 0:
            ...
            if (epoch % 2 == 0 and epoch > 0) or (epoch % 5 == 0 and epoch > cfg['decay1']):
                torch.save(net.state_dict(), save_folder + 'Angle_V4x320_conv_v1_1_new_roi_27w_epoch_' + str(epoch) + '.pth')
            epoch += 1
