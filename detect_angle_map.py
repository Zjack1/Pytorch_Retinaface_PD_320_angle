...
parser.add_argument('--trained_model', default='./weights/27w_conv_fc/Angle_V4x320_conv_fc_27w_epoch_newlr_104.pth',
...
if __name__ == '__main__':
    ...
    for line in lines:
        ...
        if 1:
            ...
            label_gt = 'GT  Y  P  R  ' + str('{0:.2f} '.format(float(gt_pry[2]) )) + str('{0:.2f} '.format(float(gt_pry[0]) )) + str(
                '{0:.2f} '.format(float(gt_pry[1]) ))
            label_loss = 'LOSS  Y  P  R  ' + str('{0:.2f} '.format(float(y_loss.item()) )) + str('{0:.2f} '.format(float(p_loss.item()) )) + str(
                '{0:.2f} '.format(float(r_loss.item()) ))
            ...
            cv2.putText(img_raw, label_loss, (40, 160), cv2.FONT_HERSHEY_SIMPLEX, 0.9,(111, 111, 1), 2)
            print(label)
            if p_loss.item() > 10:
                cv2.imwrite("./error_img/" + str(n) +".jpg",img_raw)
